...
class Test_Histogram : public TestCase
{
  virtual void run()
  {
    ...
    UINT8 vec1[16] = {50,  51,  45,  50, 50, 35, 254, 45,
                      254, 254, 254, 50, 35, 45, 254, 48};
    im1 << vec1;
    ...
    if (retVal != RES_OK) {
      ...
      for (; it1 != hist.end(); it1++, it2++)
        cout << int(it1->first) << ": " << int(it1->second) << " "
             << int(it2->second) << endl;
    }
  }
};
...
class Test_Threshold : public TestCase
{
  virtual void run()
  {
    ...
    UINT8 vec1[16] = {150, 21, 52, 50, 50, 55, 60, 45,
                      98,  54, 65, 50, 35, 59, 20, 48};
...
class Test_Stretch_Histogram : public TestCase
{
  virtual void run()
  {
    ...
    UINT8 vec1[16] = {50, 51, 52, 50, 50, 55, 60, 45,
                      98, 54, 65, 50, 35, 59, 20, 48};

    UINT8 vec2[16] = {
        98, 101, 105, 98, 98, 114, 131, 82, 255, 111, 147, 98, 49, 128, 0, 92,
    };
...
class Test_Otsu : public TestCase
{
  virtual void run()
  {
    ...
    UINT8 vec1[100] = {
        113, 77,  84,  185, 20,  55,  150, 198, 99,  49,  130, 7,   186,
        99,  233, 150, 209, 160, 203, 70,  95,  25,  79,  64,  241, 237,
        145, 50,  16,  100, 97,  219, 238, 214, 69,  29,  188, 102, 183,
        206, 221, 234, 124, 27,  6,   1,   53,  249, 107, 162, 254, 93,
        203, 36,  176, 223, 181, 88,  11,  69,  80,  250, 36,  37,  99,
        101, 91,  67,  224, 26,  10,  98,  89,  139, 118, 56,  252, 63,
        123, 169, 241, 82,  223, 215, 118, 191, 241, 212, 47,  232, 118,
        227, 128, 123, 78,  211, 95,  21,  121, 148};
