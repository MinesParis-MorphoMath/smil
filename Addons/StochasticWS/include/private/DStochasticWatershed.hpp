...
namespace smil
{
    ...
    {
        public:
            stochastic_edge(const labelT &s, const labelT &d, const double &w, const T &a, const T &l, const double &m) : source(s), dest(d), weight(w), altitude(a), length(l), dist_max(m) {}
            stochastic_edge(const stochastic_edge& s) : source(s.source), dest (s.dest), weight (s.weight), altitude(s.altitude), length(s.length), dist_max(s.dist_max) {}
            ...
            size_t length;
            double dist_max;
            labelT getSource (){return source;}
            labelT getDest (){return dest;}
            double getWeight (){return weight;}
            T getAltitude (){return altitude;}
            size_t getLength (){return length;}
            double getDistMax () {return dist_max;}
    };
    ...
    {
        public:
            stochastic_node(const size_t s, const size_t a, const double d) : area(a), surface(s), dist_max(d) {}
            map<labelT, size_t> edges;
            size_t area;
            size_t surface;
            double dist_max;
            map<labelT, size_t> getEdges(){return edges;}
            size_t getArea (){return area;}
            size_t getSurface (){return surface;}
            double getDistMax () {return dist_max;}
    };
    ...
    class stochastic_graph
    {
        public:
            ...
            vector<stochastic_edge<labelT, T> > getEdges(){return edges;}
            vector<stochastic_node<labelT> > getNodes(){return nodes;}
    };
                ...
                if (pixels[p.o] != ImDtTypes<T>::max() && pixels[p.o] != 0) {
                    ...
                    ForEachNeighborOf(p,q)
                    {
                        if (!is_done[pixels[q.o]]) {
                                if (pixels[p.o] != pixels[q.o] && pixels[q.o] != 0 && pixels[q.o] != ImDtTypes<labelT>::max()) {
                                outs[p.o] = labelT((1.-graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].weight)*double(ImDtTypes<labelT>::max()));
                             }
                        }
            ...
            ForEachPixel (p)
            {
                if (pixels[p.o] != ImDtTypes<T>::max() && pixels[p.o] != 0) {
                    ...
                    if (it != correspondances.end())
                        outs[p.o] = (markers[it->second] > 0) ? 0 : ImDtTypes<labelT>::max();
                }
                if (pixels[p.o] != 0) {
                ForEachNeighborOf(p,q)
                {
    ...
    void mosaicToStochasticGraph (const Image<labelT> &img, const Image<T>& gradI, stochastic_graph<labelT, T>& graph, const StrElt &s)
    {
...

        {
            ...
            {
                if (pixels[p.o] != ImDtTypes<T>::max() && pixels[p.o] != 0) {
                    graph.nodes[pixels[p.o]].dist_max = (sqrt(dists[p.o]) > graph.nodes[pixels[p.o]].dist_max) ? sqrt(dists[p.o]) : graph.nodes[pixels[p.o]].dist_max;
                    ...
                    ForEachNeighborOf(p,q)
                    {
                        if (!is_done[q.o] && pixels[p.o] != pixels[q.o] && pixels[q.o] != 0 && pixels[q.o] != ImDtTypes<labelT>::max()) {
                            if (graph.nodes[pixels[p.o]].edges.find(pixels[q.o]) == graph.nodes[pixels[p.o]].edges.end()) {
                                graph.edges.push_back(stochastic_edge<labelT, T> (pixels[p.o], pixels[q.o], 1., ImDtTypes<T>::max(), 0, 0));
                                graph.nodes[pixels[q.o]].edges[pixels[p.o]] = graph.edges.size()-1;
                                graph.nodes[pixels[p.o]].edges[pixels[q.o]] = graph.edges.size()-1;
                            }
                            graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].altitude = (graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].altitude > grads[p.o]) ? grads[p.o] : graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].altitude;
                            graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].dist_max = (graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].dist_max < sqrt(dists[p.o])) ? sqrt(dists[p.o]) : graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].dist_max;
                            if (!has_zero)
                                    ++graph.edges[graph.nodes[pixels[p.o]].edges[pixels[q.o]]].length;
                        }
                        is_done[pixels[q.o]] = true;
                    }
                    ENDForEachNeighborOf
   ...
   void visit_stochastic_node (const T &l, stochastic_graph<labelT, T> &r, vector<T> &visited, const size_t &i) {
        visited[i] = l;
        for (typename map<T, size_t>::iterator it=r.nodes[i].edges.begin(); it!=r.nodes[i].edges.end(); ++it) {
...
//        size_t nbr_markers = prob_dist.size();
   ...
   std::vector<size_t> watershedGraph (stochastic_graph<labelT, T> &graph, std::vector<int>& markers) {
        ...
        for (typename std::map<T, int>::iterator it=dist_altitude.begin(); it!=dist_altitude.end(); ++it) {
        ...
        std::vector<bool> already_pushed = std::vector<bool> (graph.edges.size(), false);

        labelT basin = 0;
        // Initialisation
        int i=0;
        for (typename std::vector<int>::iterator it=markers.begin(); it!=markers.end(); ++it) {
                if (*it > 0) {
                        ++basin;
                        basins[i] = basin;
                        // Push the edges out of it into the HQ.
                        for (typename std::map<labelT, size_t>::iterator jt=graph.nodes[i].edges.begin(); jt!=graph.nodes[i].edges.end(); ++jt) {
        ...
        while (it != hq.end()) {
                if (it->second.size() != 0) {
                        ...
                        if (l1 > 0 && l2 == 0) {
                                basins[p.dest] = l1;
                                for (typename std::map<labelT, size_t>::iterator jt=graph.nodes[p.dest].edges.begin(); jt!=graph.nodes[p.dest].edges.end(); ++jt) {
                                        if (!already_pushed[jt->second]) {
                                                already_pushed[jt->second] = true;
                                                hq[graph.edges[jt->second].altitude].push (jt->second);
                                        }
                                ...
                                basins[p.source] = l2;
                                for (typename std::map<labelT, size_t>::iterator jt=graph.nodes[p.source].edges.begin(); jt!=graph.nodes[p.source].edges.end(); ++jt) {
                                        if (!already_pushed[jt->second]) {
                                                already_pushed[jt->second] = true;
                                                hq[graph.edges[jt->second].altitude].push (jt->second);
                                        }
   ...
   std::vector<double> weightHierarchy (std::vector<hierarchy> &h, stochastic_graph<labelT, T>& graph) {
        ...
        for (typename std::vector<hierarchy>::iterator it=h.begin(); it!=h.end(); ++it) {
                ...
                dist_max[i] = (dist_max[it->left] > dist_max[it->right]) ? dist_max[it->left] : dist_max[it->right];
                ++i;++j;
        }
...
//                std::cerr << i << " " << surface[i] << " " << area[i] << " " << dist_max[i] << " " << out[i] << std::endl;
        }
   ...
   void cut (std::vector<double>& weights, std::vector<hierarchy>& h, stochastic_graph<labelT, T>& graph, const size_t &node) {
        ...
        if (int(weights[h[edge].left]) <= int(weight) || int(weights[h[edge].right]) <= int(weight)) {
   ...
   void stochasticWatershedParallel (const Image<labelT>& primary, const Image<T>& gradient, Image<labelT> &out, const size_t& n_seeds, const StrElt& s)
   {
        ...
        for (uint32_t i=1; i<nbr_subgraphs+1; ++i) {
                ...
                for (uint32_t j=0; j<n_seeds; ++j) {
                        ...
                        for (typename std::vector <size_t>::iterator it=ws.begin(); it!=ws.end(); ++it) {
                                mst.edges[*it].weight += 1.;
                        }
                }

                for (typename std::vector<stochastic_edge<labelT, T> >::iterator it=mst.edges.begin(); it!=mst.edges.end(); ++it) {
                        it->weight /= n_seeds;
                }

                // Copy the weights of the subgraph into the graph.
                for (typename std::vector<stochastic_edge<labelT, T> >::iterator it=mst.edges.begin(); it!=mst.edges.end(); ++it) {
                        graph.edges[graph.nodes[originals[it->source]].edges[originals[it->dest]]].weight = it->weight;
                }
        }

        stochasticGraphToPDF (primary, graph, out, s);
   }
