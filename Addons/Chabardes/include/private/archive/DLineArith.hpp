...
namespace smil
{
    ...
    {
        equLines () {}
        void _exec (const T* lIn1, const T* lIn2, const T* lIn3, const T* lIn4, const size_t size, T* lOut)
        {
            for (size_t i=0; i<size; ++i)
            {
    ...
    struct equLinesMasked
    {
        ...
        inline void _exec (T* lIn1, T* lIn2, T* lIn3, T* lIn4, const size_t size, T* lOut)
        {
...
/*
    ...
    struct equLines : public tertiaryLineFunctionBase<T1,T2,T2,T2>
    {
        ...
        typedef typename tertiaryLineFunctionBase<T1,T2,T2,T2>::lineType1 lineType1;
        typedef typename tertiaryLineFunctionBase<T1,T2,T2,T2>::lineOutType lineType2;
    ...
    struct equLines2 : public binaryLineFunctionBase<T>
    {
        ...
        virtual void _exec(const lineType1 lIn1, const lineType2 lIn2, const size_t size, lineType2 lOut)
        {
            for (size_t i=0;i<size;i++)
...
*/
    ...
    struct supLines : public binaryLineFunctionBase<T1,T1,T2>
    {
        ...
        virtual void _exec(const lineType1 lIn1, const lineType1 lIn2, const size_t size, lineType2 lOut)
        {
            for (size_t i=0;i<size;i++)
