...
namespace smil
{
    ...
    RES_T areaThreshold (const Image<T> &_im_, const T &threshold, Image<T> &_out_)
    {
        ...
        for (typename map<T, double>::iterator it=m.begin(); it!=m.end();++it)
        {
        ...
        for (z=0; z<size[2]; ++z) {
            ...
            for (x=0; x<size[0];++x) {
                ...
                for (y=1; y<size[1]; ++y) {
                    ...
                    } else if (pixelsIn[offset+y*size[0]] != pixelsIn[offset+(y-1)*size[0]]) {
                ...
                for (y=size[1]-2; y>=0; --y) {
                    if (pixelsIn[offset+(y+1)*size[0]] != pixelsIn[offset+y*size[0]]) {
                        min = 1;
                    }
                    else {
            ...
            for (x=0; x<size[0]; ++x) {
                ...
                for (z=1; z<size[2]; ++z) {
                    if (pixelsOut[offset+z*size[1]*size[0]] != 0) {
                        if (pixelsIn[offset+z*size[1]*size[0]] != pixelsIn[offset+(z-1)*size[1]*size[0]]) {
                            pixelsOut[offset+z*size[1]*size[0]] = T2(1);
                        } else if (pixelsOut[offset+z*size[1]*size[0]] > pixelsOut[offset+(z-1)*size[1]*size[0]]) {
                ...
                while (!level->empty ()) {
                        ...
                        while (pt!=sePoints.end ()) {
                                ...
                                if (   n_x >= 0 && n_x < (int)size[0] &&
                                       ...
                                       pixelsIn [n_x + (n_y)*size[0] + (n_z)*size[1]*size[0]] == pixelsIn[x + y*size[0] + z*size[1]*size[0]])
                                {
    ...
    RES_T find_triple_points (const Image<T1> &_im_, const Image<T2> &_skiz_, Image<T2> &_out_, const UINT& val, const StrElt& _se_)
    {
        ...
        #pragma omp parallel num_threads(nthreads)
        {
            ...
            ForEachPixel (p)
            {
                ...
                if (m.size() >= val && skiz[p.o] != 0 && skiz[p.o] != ImDtTypes<T2>::max())
                {
