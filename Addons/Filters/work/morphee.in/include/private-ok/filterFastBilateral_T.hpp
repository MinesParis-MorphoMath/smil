...
namespace morphee
{
  ...
  {
    template <class Tin>
    void FastBilateralFilter(Tin *T, int W, int H, int Z, int methodS,
    ...
    // spatial S et niveau de gris G Param: nS(E) taille voisinage (à
    // l'extérieur, Gaussienne nulle) Param: EcTSx,y,z(E) ecart type Gaussienne
    // filtrage spatial suivant axes x, y et z Param: EcTGx,y,z(E) Ecart type
    // Gaussienne filtrage niveau de gris suivant axes x, y et z Retour: Vint 3D
    // Remarque: calcul inexact (car le filtre n'est pas décomposable), mais les
    // résultats sont corrects et beaucoup plus rapide Possibilité de gérer
    // différement le filtrage suivant les axes valeur typique EcTS=3ou5
    // EcTG=20ou40 Equivalence Gauss - Tukey: Ect - Ect*Rac(5) Exemple de
    // paramètres correct: D=FastBilateralFilter(D,W,H,Z,2,2,10,3,3,3,20,20,20);
    // ou
    ...
    // mais resultat encore meilleurs)
    {
      ...
      if (methodS != 1) // Tukey correction (pour correspondre à un écart type
                        // de Gaussienne, plus intuitif)
      {
        EcTSx *= sqrt(5.0);
        EcTSy *= sqrt(5.0);
        EcTSz *= sqrt(5.0);
    ...
    {
      MORPHEE_ENTER_FUNCTION("t_ImFastBilateralFilter");
      // Check inputs
      ...
      FastBilateralFilter(imOut.rawPointer(), W, H, Z, Method, Method, nS, EctS,
    ...
    RES_C t_ImFastBilateralFilterRGB(const Image<T1> &imIn, const UINT8 Method,
                                     ...
                                     const UINT32 EctG, Image<T2> &imOut)
    {
      MORPHEE_ENTER_FUNCTION("t_ImFastBilateralFilterRGB");
      RES_C res;
      ...
      if (res != RES_OK) {
        MORPHEE_REGISTER_ERROR(
            "Error t_colorSplitTo3() in t_ImFastBilateralFilterRGB ");
        return res;
      }
      ...
      FastBilateralFilter(imInR.rawPointer(), W, H, Z, Method, Method, nS, EctS,
                          EctS, EctS, EctG, EctG, EctG);
      FastBilateralFilter(imInG.rawPointer(), W, H, Z, Method, Method, nS, EctS,
                          EctS, EctS, EctG, EctG, EctG);
      FastBilateralFilter(imInB.rawPointer(), W, H, Z, Method, Method, nS, EctS,
                          EctS, EctS, EctG, EctG, EctG);
      //****************************************************************************************
      ...
      if (res != RES_OK) {
        MORPHEE_REGISTER_ERROR(
            "Error colorComposeFrom3() in t_ImFastBilateralFilterRGB ");
        return res;
      }
