...
#define WINDOW_SIZE 7
    ...
    void debed(T2 *imOut, int W, int H, int size, double *Img, int NewW,
               int NewH)
    {
      int i, j;
      size += 2;
                                   ...
                                   double Alpha)
    {
      ...
      for (j = 0; j < H; j++) {
        ...
        Causal[0 + j * W] =
            s1; // recopie du point dans les 2 premieres colonnes.
        Causal[1 + j * W] = s2;
                                 ...
                                 double *AntiCausal, int W, int H, double Alpha)
    {
      ...
      for (i = 0; i < W; i++) {
        ...
        for (j = 2; j < H; j++) {
          ...
          s1 = s;
          s1 = g * x[i + j * W] + g * a1 * c2 * x[i + (j - 1) * W] +
               2 * a1 * s1 -
               a2 * s2; // deuxieme lissage causal pour initialisation
          s2 = s;       // de l'anticausal
        }
    ...
    bool *ComputeBli(double *ImgFiltrer, double *ImgBuf, int W, int H)
    {
      ...
      for (i = 0; i < W; i++)
        for (j = 0; j < H; j++) {
          ImgBli[i + j * W] = 0;
          if (i < OUTLINE || i >= W - OUTLINE || j < OUTLINE ||
              j >= H - OUTLINE)
            continue;
          ImgBli[i + j * W] =
              ((ImgFiltrer[i + j * W] - ImgBuf[i + j * W]) > 0.0);
        }
    ...
    //*************************************************************************************
    double ComputeAdaptativeGradient(bool *Bli, double *Orig, int W, int H,
                                     int k, int l)
    {
      int i, j;
      double SumOn, SumOff;
      ...
      // On regarde par rapport aux pixels voisins, le nombre de pixel a 1 et a
      // 0
                                   ...
                                   Image<T2> &imOut)
    {
      ...
      if (Alpha < 0 || Alpha > 1) {
        MORPHEE_REGISTER_ERROR(
            "The parameter Alpha must have a value between 0 and 1");
      ...
      return Deriche(imIn.rawPointer(), W, H, Alpha, imOut.rawPointer());
    }
