...
#define PI 3.14159
    ...
    void SeparableConvolution(T1 *imIn, int Width, int Height, float *gau,
    ...
    //****************************************************************************
    void SeparableConvolution_dxy(int Width, int Height, float *Image,
    ...
    void NonMaxSuppress(int Width, int Height, float *dX, float *dY,
                        T2 *imNorme)
    {
      int i, j;
      ...
      for (i = 0; i < Width; i++)
        for (j = 0; j < Height; j++) {
          ...
          if (fabs(VectY) >
              fabs(VectX)) // la direction du vecteur est verticale
          {
            ...
            if (VectX * VectY > 0) {
              N3 = Norm(dX[i + 1 + (j + 1) * Width],
                        dY[i + 1 + (j + 1) * Width]);
              N1 = Norm(dX[i - 1 + (j - 1) * Width],
                        dY[i - 1 + (j - 1) * Width]);
            } else {
              N3 = Norm(dX[i + 1 + (j - 1) * Width],
                        dY[i + 1 + (j - 1) * Width]);
    ...
    RES_C Canny(T1 *imIn, int W, int H, double Sigma, T2 *imNorme)
    {
      float Gaussien[MAX_SIZE_MASK], dGaussien[MAX_SIZE_MASK];
      int i;
      float *dX = 0, *dY = 0, *FlouX = 0, *FlouY = 0;
      int taille = 0;
    ...
    RES_C t_ImCannyEdgeDetection(const Image<T1> &imIn, const double Sigma,
                                 Image<T2> &imOut)
    {
      // Check inputs
