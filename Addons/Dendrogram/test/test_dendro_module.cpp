...
#include "DendroModule.h"
#include <iostream>
#include <map>

using namespace smil;

class Test_Dendrogram : public TestCase {
  virtual void run() {
    // Construction dendrogramme exemple
    // 	Dendrogram<UINT16,UINT16,UINT16> dendroTest(11);
    // 	DendroNode<UINT16>
    // dendroNodeA,dendroNodeB,dendroNodeC,dendroNodeD,dendroNodeE,
    ...
    // 	dendroNodeA.setMarker(0); //already default value, just to make things
    // clearer
    ...
    //     std::vector<DendroNode<UINT16>*> dendroTestNodes =
    //     dendroTest.getDendroNodes();
    ...
    //     std::vector<DendroNode<UINT16>*> dendroTestNodesAfterHierarchy =
    //     dendroTest.getDendroNodes();
    //     for (int i=0;i<dendroTestNodesAfterHierarchy.size();i++){
    // 		cout << "dendroTestNodesAfterHierarchy["<<i
    // 		<<"].getValuation = "
    // 		<< dendroTestNodesAfterHierarchy[i]->getInternalNodeValuationFinal() <<
    // endl;
    // 	}
    //
...
//     Image<UINT8> imIn = smil::Image<UINT8>(
//         "/home/afehri/Dropbox/Images/imTestPerso/cameramanMosaic.png");
...
//     mosaicToGraph(imMosaic, imEdgeValues, imNodeValues,
//                   g2); // imEdgeValues, imNodeValues,
...
//     cout << "STEP 2 complete : MST computation" << endl;
    ...
    // 	Dendrogram<UINT16,UINT16,UINT16> dendro1 =
    // Dendrogram<UINT16,UINT16,UINT16>(MST1);
//     Dendrogram<UINT32, UINT32, UINT32> dendro2 =
//         Dendrogram<UINT32, UINT32, UINT32>(MST2);

    //     cout << " \n *********TEST_SORT_NODES********" << endl;
    /*    dendro1.sortNodes(true);
        std::vector<DendroNode<UINT16>*> dendro1Nodes =
       dendro1.getDendroNodes();
        for (int i=0;i<dendro1Nodes.size();i++){
                    cout << "dendroTestNodes["<<i
                    <<"].getValuation= "
                    << dendro1Nodes[i]->getInternalNodeValuationInitial() <<
       endl;
            }*/

    //     dendro2.sortNodes(true);
    //     std::vector<DendroNode<UINT16>*> dendro2Nodes =
    //     dendro2.getDendroNodes();
...
//     cout << "STEP 4 complete : Modifications of the dendrogram " << endl;
    ...
    // 	dendro2.sortNodes(true);//sort Nodes of dendro2 by decreasing
...
//     string
//         name_image = "/home/afehri/ownCloud/Images/imTestPerso/"
//                      "cameramanMosaic.png"; //"/home/afehri/Dev/Results/toolsHierarchy/volumic/images/tools";
//     // std::vector<DendroNode<UINT32> *> &dendroNodes2 = dendro2.getDendroNodes();
// 
//     dendro2.removeMSTEdgesDendrogram(MST2, lambda);
...
//     Gui::execLoop();
    ...
    // TEST
    // 	map<long unsigned int, long unsigned int > &G1Nodes =
    // g1.getNodeValues();
    // 	map<long unsigned int, long unsigned int > &G2Nodes =
    // g2.getNodeValues();
    // 	map<long unsigned int, long unsigned int > &MST2Nodes =
    // MST2.getNodeValues();
    ...
    //        MapIterator my_map;
    //        for( MapIterator::iterator it = MST2Nodes.begin(); it !=
    //        MST2Nodes.end(); ++it ) //traversal of the MST edges by growing
    //        values of edges
    // 	    {
    // 	      int key = it->first;
    // 	      int value = it->second;
    ...
    //        Graph<> graphAfterHierarchy =
    //        Dendrogram::dendroToGraph(dendroFromMST);
    ...
    // 	}
    //
    // 	vector< Edge<long unsigned int>,allocator<Edge<long unsigned int> > >
    // MST1edges = MST1.getEdges();
    ...
    //        std::vector<DendroNode*>&dendroFromMSTNodes =
    //        dendroFromMST.getDendroNodes();
    //        cout << dendroFromMST.getDendroNodes().size() << endl;
    //        DendroNode &curNode = *dendroFromMSTNodes[5];//leavesNbr+i
    //
    //
    //        map<long unsigned int,vector<long unsigned int> >mstNodeEdges =
    //        mst.getNodeEdges();
    //        typedef map<long unsigned int, vector<long unsigned int> >
    //        MapIterator;
    //        int counter=0;
    //        MapIterator my_map;
    //        for( MapIterator::iterator it = mstNodeEdges.begin(); it !=
    //        mstNodeEdges.end(); ++it ) //traversal of the MST edges by growing
    //        values of edges
    // 	    {
    // 	      int key = it->first;
...
class Test_DendrogramConstructor : public TestCase {
  virtual void run() {
    ...
    Image<UINT8> im8 = smil::Image<UINT8>("/home/fehri/Images/lena.png");
    Image<UINT8> imIn = smil::Image<UINT8>("/home/fehri/Images/lenaMosaic.png");
    
    Image<UINT8> imGrad(imIn);

    Image<UINT32> imLabels = smil::Image<UINT32>(imIn);
    Image<UINT32> imOut = smil::Image<UINT32>(imIn);
    label(imIn, imLabels);
    //     imLabels.show();
    //     smil::labelWithVolume<UINT8,UINT32>(im8,imLabels,imOut);
    //     imLabels.show();
    //     smil::labelWithArea<UINT8,UINT32>(imLabels,imOut);
    //     imOut.show();
    //     Gui::execLoop();
    //
    Image<UINT32> imMosaic(imIn);
    Image<UINT32> imMosaic2(imIn);
    Image<UINT32> imIn32(imIn);
    ...
    labelWithArea(imLabels, imNodeValues);
    
    gradient(imIn, imGrad);
    copy(imGrad, imEdgeValues);
    
    UINT32 un = 1;
    label(imLabels, imMosaic2);
...
//     mst2.printSelf();
    //     Dendrogram<UINT32,UINT32,UINT32> dendroFromG2 =
    //     Dendrogram<UINT32,UINT32,UINT32>(mst2);
//     cout << mst2.getNodeNbr() << endl;
//     cout << maxVal(imMosaic2) << endl;
//     Gui::execLoop();
    
    Dendrogram<UINT32, UINT32, UINT32> dendroFromG2Energy 
	  = Dendrogram<UINT32, UINT32, UINT32>(mst2, imMosaic2, imIn32, 3);
    
//     Dendrogram<UINT32, UINT32, UINT32> dendroFromG2Energy = Dendrogram<UINT32, UINT32, UINT32>(mst2);
    cout << "dendroFromG2Energy created" << endl;
    
       dendroFromG2Energy.HierarchicalConstruction("none",100,imMosaic2,"erode");

    Dendrogram<UINT32, UINT32, UINT32> dendroTmp =  Dendrogram<UINT32, UINT32, UINT32>(dendroFromG2Energy);
    dendroTmp.HierarchicalConstruction("stochasticSurfacic",100,imMosaic2,"erode");
    
    cout << "H1 created" << endl;
    
    size_t nbrMarkers = dendroTmp.getNbrMarkers();
    cout << nbrMarkers << endl;
    
//     std::vector<MarkerLabelT> nMarkersCount(nbrMarkers, 0);
//     dendroFromG2Energy.HierarchicalConstruction("stochasticSurfacic",100,imMosaic2,"erode");
//     cout << "H2 created" << endl;
//     dendroFromG2Energy.EnergyConstruction("simplifiedMumfordShah", 0);
//     cout << "H3 created" << endl;
//     dendroFromG2Energy.normalize();
  }
};
