...
template <class ImageLabel, class ImageVal, class ImageMarker, class SE,
          class ImageOut>
RES_T GeoCuts_MinSurfaces_with_steps(const ImageLabel &imLabel,
                                     ...
                                     F_SIMPLE step_z, ImageOut &imOut)
{
  MORPHEE_ENTER_FUNCTION(
      "t_GeoCuts_MinSurfaces_with_steps (multi-valued version)");
...
RES_T MAP_MRF_edge_preserving(const ImageIn &imIn,
                                const ImageMosaic &imMosaic,
                                const ImageMarker &imMarker, const _Beta Beta,
                                const _Sigma Sigma, const SE &nl,
                                ImageOut &imOut)
{
    ...
    for (nit = neighb.begin(), nend = neighb.end(); nit != nend; ++nit) {
      ...
      if (o2 > o1) {
        if (val != val2) {
          ...
          double cost =
              (double) Beta -
              ((double) Beta) * std::pow(std::abs(mean[val] - mean[val2]), 6.0);
          if (in1 == 0) {
            boost::tie(e4, in1) = boost::add_edge(val, val2, g);
            boost::tie(e3, in1) = boost::add_edge(val2, val, g);
            capacity[e4]        = cost;
