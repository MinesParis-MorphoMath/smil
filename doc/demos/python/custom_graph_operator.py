      ...
      for nb in relOffList:
          ...
          if curVal!=nbVal:
              ...
              if eInd==-1: # not found -> create a new one
                  if type(self.imNodeValues)!=type(None): # If an image with node values is given
                      self.graph.addNode(curVal, self.imNodeValues[i])
                      self.graph.addNode(nbVal, self.imNodeValues[i+nb])
                  if type(self.imEdgeValues)!=type(None): # If an image with edge values is given
                      self.borderValues.append([self.imEdgeValues[i]])
                  self.graph.addEdge(curVal, nbVal, 0, False) # False -> avoids to check if the edge already exists (done previoulsy)
