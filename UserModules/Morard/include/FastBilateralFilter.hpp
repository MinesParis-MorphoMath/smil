...
namespace smil
{
        ...
        template<class Tin>
                ...
                //Param: nS(E) taille voisinage (� l'ext�rieur, Gaussienne nulle)
                ...
                //Retour: Vint 3D
                //Remarque: calcul inexact (car le filtre n'est pas d�composable), mais les r�sultats sont corrects et beaucoup plus rapide
                //Possibilit� de g�rer diff�rement le filtrage suivant les axes
                //valeur typique EcTS=3ou5 EcTG=20ou40
                //Equivalence Gauss - Tukey: Ect - Ect*Rac(5)
                //Exemple de param�tres correct:
                ...
                //D=FastBilateralFilter(D,W,H,Z,2,2,10,5,5,5,20,20,20); (un peu plus lent mais resultat encore meilleurs)
        {
                ...
                if (methodS!=1)//Tukey correction (pour correspondre � un �cart type de Gaussienne, plus intuitif)
                {
                        EcTSx*=sqrt(5.0);
                        EcTSy*=sqrt(5.0);
                        EcTSz*=sqrt(5.0);
