...
struct _SMIL unaryLineFunctionBase
{
    ...
    inline void operator()(T *lineIn, int size, T *lineOut, UINT misAlignSize=0)
    { 
	if (misAlignSize)
	  _exec(lineIn, misAlignSize, lineOut); 
	_exec_aligned(lineIn+misAlignSize, size-misAlignSize, lineOut+misAlignSize); 
    }
    inline void operator()(T *lineIn, int size, T value, UINT misAlignSize=0)
    { 
	if (misAlignSize)
...
struct _SMIL binaryLineFunctionBase
{
    ...
    inline void operator()(T *lineIn1, T *lineIn2, int size, T *lineOut, UINT misAlignSize=0)
    { 
	if (misAlignSize)
...
struct _SMIL tertiaryLineFunctionBase
{
    ...
    inline void operator()(T *lineIn1, T *lineIn2, T *lineIn3, int size, T *lineOut, UINT misAlignSize=0)
    { 
	if (misAlignSize)
